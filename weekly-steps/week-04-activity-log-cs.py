...
MAIN_TITLE = "Historie aktivit: Týden 4 - Zpracování chyb"
DEVELOPER = "HMP"
...
def run_script():
    ...
    s = get_styles()
    story = []
    content = [{'id': 1, 'title': 'I. Základní koncepty', 'paragraphs': ['Tato sekce pokrývá základní koncepty Zpracování chyb. Základní principy vytvářejí základ pro pochopení pokročilých technik. Klíčová terminologie a definice jsou představeny.', 'Teoretický základ poskytuje kontext pro praktické aplikace. Pochopení těchto konceptů umožňuje efektivní implementaci.', 'Stavební bloky jsou stanoveny pro praktická cvičení. Každý koncept se propojuje s reálnými vývojovými scénáři.'], 'label': 'Klíčové body', 'items': ['Základní principy Zpracování chyb', 'Klíčová terminologie a definice', 'Teoretický základ stanoven', 'Propojení s předchozím učením']}, {'id': 2, 'title': 'II. Detaily implementace', 'paragraphs': ['Implementace Zpracování chyb následuje zavedené vzory. Příklady kódu demonstrují praktické použití. Syntaxe a struktura jsou podrobně vysvětleny.', 'Běžné vzory a idiomy jsou prezentovány. Zpracování chyb a hraniční případy jsou řešeny.', 'Průvodce implementací krok za krokem vede proces učení. Organizace kódu následuje osvědčené postupy.'], 'label': 'Technické shrnutí', 'items': ['Implementační vzory demonstrovány', 'Příklady kódu s vysvětlením', 'Zpracování chyb řešeno', 'Výkonnostní aspekty zmíněny']}, {'id': 3, 'title': 'III. Praktické aplikace', 'paragraphs': ['Praktické aplikace Zpracování chyb řeší reálné problémy. Případy použití z profesionálního vývoje jsou prozkoumány. Integrace s existujícími kódovými základnami je diskutována.', 'Běžné scénáře demonstrují typické použití. Techniky ladění a řešení problémů jsou pokryty.', 'Příklady z reálného světa ilustrují klíčové koncepty. Osvědčené postupy z průmyslové praxe jsou sdíleny.'], 'label': 'Vzory použití', 'items': ['Reálné případy použití', 'Integrační strategie', 'Techniky ladění', 'Optimalizační přístupy']}, {'id': 4, 'title': 'IV. Projekt a osvědčené postupy', 'paragraphs': ['Týdenní projekt aplikuje koncepty Zpracování chyb. Požadavky jsou jasně definovány. Implementace následuje naučené vzory.', 'Testování validuje implementaci. Dokumentace zachycuje designová rozhodnutí.', 'Dokončený projekt demonstruje zdatnost. Dovednosti se přenášejí do budoucích projektů.'], 'label': 'Implementace projektu', 'items': ['Požadavky projektu definovány', 'Implementace dokončena', 'Testování validuje správnost', 'Dokumentace poskytnuta']}]
